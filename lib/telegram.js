...
async function pollTelegramReply(config, topicId, timeoutSeconds) {
  ...
  while (Date.now() < deadline) {
    ...
    for (const update of (updates.result || [])) {
      ...
      // Check it's from the right group
      if (String(msg.chat?.id) !== String(groupId)) continue;

      // Check it's in the right topic thread
      if (topicId && msg.message_thread_id !== topicId) continue;
